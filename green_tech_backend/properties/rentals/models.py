"""
Rental property management models for the GreenTech Africa platform.
"""
...
class RentalProperty(models.Model):
    ...
    def clean(self):
        ...
        if self.available_to and self.available_from and self.available_to < self.available_from:
            raise ValidationError({
                'available_to': _('Available to date must be after available from date.')
            })
            
        if self.maximum_lease_months and self.minimum_lease_months > self.maximum_lease_months:
            raise ValidationError({
                'maximum_lease_months': _('Maximum lease months must be greater than or equal to minimum lease months.')
            })
    ...
    def can_be_rented_for_duration(self, start_date, end_date):
        """Check if the property is available for the specified duration."""
        if not self.is_available:
            return False
        ...
        if self.available_to and end_date > self.available_to:
            return False
            
        # Check for overlapping leases
        overlapping_lease = self.property.lease_agreements.filter(
...
class LeaseAgreement(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Set end date for fixed-term leases if not provided
        if (self.lease_type == LeaseType.FIXED_TERM and 
            self.start_date and 
            not self.end_date):
            self.end_date = self.start_date + timedelta(days=365)  # Default 1-year lease
